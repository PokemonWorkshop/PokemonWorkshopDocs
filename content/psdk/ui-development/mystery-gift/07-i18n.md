...
This guide builds on the [mouse guide](/psdk/ui-development/mystery-gift/handle-mouse-input). The [setup guide](/psdk/ui-development/mystery-gift/prepare-ui-assets) already installed the CSV file, and we declared the `TEXT_*` constants back in the [composition guide](/psdk/ui-development/mystery-gift/create-a-composition). Here we explain how the code reads that CSV, add a helper to keep the calls short, and replace our last hardcoded string.
...
- The A button stays hidden (`nil`) for now. We reveal it -- with its `gift_text(TEXT_ENTER_CODE)` label -- in the [dialogs guide](/psdk/ui-development/mystery-gift/use-confirmation-dialogs), at the same time as the `action_a` it triggers. (A visible ctrl button calls its action on click, so it must not appear before its action exists.)

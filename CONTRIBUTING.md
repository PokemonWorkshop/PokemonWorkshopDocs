...
- **description**: one or two sentence summary, in **plain text** — no Markdown. This field feeds the SEO `<meta name="description">` tag, which renders it as raw text, so `**bold**` or `` `code` `` markers would show up literally. The page's first paragraph repeats the same wording (see below).

...
export default function DocItemLayout(props: Props) {
  ...
  const rubyChapterLabel = translate({
    ...
    message: "Chapter",
    description:
      "Label prefix for the Ruby course chapters in the progress tracker",
  });
  ...
  const uiDevStepLabel = translate({
    ...
    description:
      "Label prefix for the UI Development tutorial steps in the progress tracker",
  });
  ...
  return (
    <>
      ...
      {metadata.sourceDirName === "ruby" && (
        <ProgressTracker storageKey="ruby-progress" label={rubyChapterLabel} />
      )}
